...
Generation::Generation(int rank) :  solutions(new Chromosome[POPULATION_SIZE]), newPopulation(new Chromosome[POPULATION_SIZE])
{
...
#pragma omp parallel for shared(elite1,elite2,highestFitness)
...
{
#pragma omp parallel for reduction(+:newPopulationCount) shared(newPopulation)
	for(int i=0;i<n;i++)
	{
...
#pragma omp parallel for shared(cityIndex)
