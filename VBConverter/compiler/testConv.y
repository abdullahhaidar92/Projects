...
declaration : type varList  { if(strcmp($2.varWithoutValues,"")==0)
									...
									else if($2.varWithoutValues[strlen($2.varWithoutValues)-2]==',')
											str=concat($2.varWithoutValues,$2.varWithValues);
										else{
											$2.varWithValues[strlen($2.varWithValues)-2]='\0';
											str=concat(concat($2.varWithoutValues,", "),$2.varWithValues);
										}
...
varList : ID ',' varList { verifyDoesntExist($1);
						  $$.varWithoutValues = concat(concat($1," , "),$3.varWithoutValues);
						  $$.varWithValues=$3.varWithValues;
						 }
| ID '=' arithmaticExpression ','  varList { verifyDoesntExist($1);
							    temp = concat(concat($1," As "),concat(inhType," = " ));
							    $$.varWithValues = concat(concat(temp,$3),concat(" , ",$5.varWithValues));          
							  $$.varWithoutValues=$5.varWithoutValues;
										   }
| ID {verifyDoesntExist($1);
	  $$.varWithoutValues = concat(concat($1," As "),inhType);
	  $$.varWithValues="";
	 }
